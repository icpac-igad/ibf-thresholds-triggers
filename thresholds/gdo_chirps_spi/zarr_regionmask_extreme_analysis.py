#!/usr/bin/env python3
"""
Zarr-based Regional Extreme Value Analysis using RegionMask
================================================================

This script integrates:
1. Zarr data loading for SPI datasets
...
Usage:
    python zarr_regionmask_extreme_analysis.py
"""
...
def setup_coiled_cluster(software_env="v3-geosfm-rm-x", n_workers=3):
    ...
    try:
        cluster = coiled.Cluster(
            ...
            worker_options={
                "security": {
                    "key_path": SERVICE_ACCOUNT_FILE  # Pass credentials to workers
                }
            }
        )
...
def load_icechunk_spi_data():
    """Load SPI data from Zarr store"""
    print(f"\n{'='*70}")
    print("LOADING SPI DATA FROM ZARR")
    print(f"{'='*70}")
    ...
    try:
        ...
        zarr_path = 'gs://cdi_arco/spi48_spi48.zarr'
        
        print(f"Zarr path: {zarr_path}")
        print(f"Group: spi48_data")
        ...
        print(f"✅ Successfully loaded SPI dataset from Zarr")
        ...
        print(f"❌ Failed to load Zarr data: {e}")
        raise
...
def extract_annual_extremes_for_region(spi_data, region_mask, region_id):
    """Extract annual extremes for a specific region using xclim methods"""
    try:
        # Extract regional data first
        region_data = spi_data.where(region_mask == region_id)
        
        # Add units attribute for xclim compatibility (SPI is dimensionless)
        region_data.attrs['units'] = '1'
        ...
        print(f"❌ Failed to extract annual extremes for region {region_id}: {e}")
        raise


def process_region_complete(spi_data, region_mask, region_id, region_name, return_periods):
    """Process a single region: extract annual extremes and calculate return periods - runs on worker"""
    try:
        import xarray as xr
        import numpy as np
        from xclim.indices import stats
        ...
        # Step 1: Extract annual extremes for this specific region
        region_data = spi_data.where(region_mask == region_id)
        ...
        # Step 2: Calculate spatial mean over the region
        # Compute values on the worker (avoiding serialization issues)
        region_values = annual_minima.values  # Get numpy array directly
...
def calculate_return_periods_per_region(spi_data,
                                        region_mask,
                                        gdf,
                                        client,
    ...
    print(f"SPI data dimensions: {spi_data.dims}")
    print(f"Current chunks: {spi_data.chunks}")
    
    # Determine coordinate names (could be lat/lon or latitude/longitude)
    if 'latitude' in spi_data.dims:
        chunk_dict = {'time': -1, 'latitude': 50, 'longitude': 50}
    elif 'lat' in spi_data.dims:
        chunk_dict = {'time': -1, 'lat': 50, 'lon': 50}
    else:
        # Fallback - just chunk time dimension
        chunk_dict = {'time': -1}
    
    print(f"Rechunking SPI data with: {chunk_dict}")
    spi_data = spi_data.chunk(chunk_dict)
    print(f"New chunks: {spi_data.chunks}")
    
    # Don't persist Zarr data directly - avoid serialization issues
    # Instead, we'll work with the data locally and distribute the computation
    print("Skipping persist() to avoid Zarr serialization issues")
    ...
    for region_id in unique_regions:
        ...
        # Submit the complete region processing as a delayed task
        # Pass the original SPI data and region mask to workers
        task = dask.delayed(process_region_complete)(
            spi_data, region_mask, region_id, region_name, return_periods
        )
        tasks.append(task)

    # Compute all tasks in parallel using the client
    print(f"Computing {len(tasks)} complete region processing tasks in parallel...")
...
def main():
    ...
    print("=" * 70)
    print("ZARR REGIONMASK EXTREME VALUE ANALYSIS")
    print("=" * 70)
    ...
    try:
        ...
        # Step 2: Load SPI data from Zarr
        ...
        # Step 5: Calculate return periods per region (annual extremes calculated per region in workers)
