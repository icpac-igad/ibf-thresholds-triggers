...
Contingency Tables and Forecast Verification in Drought Anticipatory Action
---------------------------------------------------------------------------

Integrating Observation and Forecast Data
-----------------------------------------

In drought early warning systems, the process of combining observations and forecasts involves several key steps:

1. **Data Preparation**: Both observed (CHIRPS) and forecasted (SEAS51) data are processed to calculate SPI (Standardized Precipitation Index) values.
2. **Threshold Application**: A threshold (typically SPI ≤ -1 for severe drought) is applied to both datasets to create binary events.
3. **Probability Extraction**: For forecasts, the percentage of ensemble members exceeding the threshold is calculated as the forecast probability.
4. **Trigger Value Testing**: Different probability thresholds are tested to determine optimal trigger values for initiating anticipatory actions.

Understanding Contingency Tables
--------------------------------

A contingency table is a 2×2 matrix that summarizes the relationship between observed events and forecasted events:
...
.. table:: Contingency Table
  ...
  +=================+================+===============+
  | Forecasted: Yes | a (Hit)        | b (False      |
  |                 |                | Alarm)        |
  +-----------------+----------------+---------------+
  | Forecasted: No  | c (Miss)       | d (Correct    |
  |                 |                | Rejection)    |
  +-----------------+----------------+---------------+

Where:

- **a (Hit)**: Drought was forecasted and occurred
- **b (False Alarm)**: Drought was forecasted but did not occur
- **c (Miss)**: Drought was not forecasted but occurred
- **d (Correct Rejection)**: Drought was not forecasted and did not occur

In the code, this table is generated by comparing binary observations (observed SPI ≤ threshold) with binary forecasts (forecast probability ≥ trigger value).
...
The CSV output file contains verification metrics calculated from the contingency table. Here's an explanation of each column:

.. list-table:: CSV Output Columns
  :widths: 30 70
  :header-rows: 1
  ...
  * - Unnamed: 0, index
    - Row identifiers
  * - x2d_region
    - Region identifier (e.g., 0 for Karamoja)
  * - x2d_leadtime
    - Lead time in months (forecast horizon)
  * - x2d_season
    - Season being forecasted (e.g., "mam" for March-April-May)
  * - x2d_level
    - Drought severity level (mild, mod, sev/ext)
  * - trigger_value
    - Probability threshold value being tested (0-100%)
  * - time_step
    - Specific time step or window being analyzed
  * - hit_rate
    - Proportion of observed events that were correctly forecasted (a/(a+c))
  * - false_alarm_ratio
    - Proportion of false alarms in all forecasts (b/(a+b))
  * - bias_score
    - Ratio of forecasted to observed events ((a+b)/(a+c))
  * - hanssen_kuipers_score
    - Difference between hit rate and false alarm rate (a/(a+c) - b/(b+d))
  * - heidke_skill_score
    - Accuracy relative to random chance
  * - total
    - Total number of cases (a+b+c+d)
  * - auroc_score
    - Area Under ROC curve - measures discrimination ability
  * - auroc_lb, auroc_ub
    - Lower and upper confidence bounds for AUROC
  * - cat
    - Drought category (mild, mod, sev/ext)
  * - obs_count
    - Number of observed drought events
  * - threshold
    - SPI threshold value used for event definition
  * - hits
    - Number of hits (a)
  * - misses
    - Number of misses (c)
  * - FA
    - Number of false alarms (b)
  * - CN
    - Number of correct negatives (d)
  * - hit_percentage
    - Percentage of observed events that were correctly forecasted
